...
CKPT = Path(sys.argv[1] if len(sys.argv) > 1 else
            '/home/jhlusko/workspace/homr-artifacts/models/ocr/crnn-h48.pth')
LIMIT = int(sys.argv[2]) if len(sys.argv) > 2 else 2000

def cer(truth, pred):
    if not truth: return 0.0 if not pred else 1.0
    d = range(len(pred) + 1)
...
for img_path, box, text, _ in samples:
    ...
    if crop.size == 0 or crop.shape[0] < 4: skipped += 1; continue
    ...
    buckets[key][2] += 1
    if len(examples) < 12: examples.append((truth, pred))
