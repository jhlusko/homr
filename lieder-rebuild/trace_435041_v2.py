...
# split into measures like TupletParser.parse does
measures = TupletParser.split_into_measures(groups)
for m_idx, measure_groups in enumerate(measures):
    result = TupletParser.add_tuplets(list(measure_groups))  # copy so we can inspect without reverting semantics confusion
    ...
    for i, g in enumerate(measure_groups):
        durs = []
        for s in g.symbols:
            if s.rhythm.startswith(("note", "rest")):
                d = s.get_duration()
                durs.append((s.rhythm, d.actual_notes, d.normal_notes, s.position))
        if durs or g.tuplet_mark:
            print(" ", i, durs, "mark=", repr(g.tuplet_mark))
